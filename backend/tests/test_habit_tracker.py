...
class TestCreateHabit:
    ...
    def test_habit_with_custom_frequency(self):
        ...
        habit = create_habit(
            "Weekly Yoga", "exercise", frequency="weekly", target_count=3
        )
    ...
    def test_habit_with_description(self):
        ...
        habit = create_habit(
            "Evening Walk", "exercise", description="30-minute walk after dinner"
        )
...
class TestGetTodayHabitForecast:
    ...
    def test_tracks_completions(self):
        ...
        assert result["habits"][0]["is_completed"]
...
class TestGetLunarCycleReport:
    ...
    def test_with_completions(self):
        ...
        habits = [
            {"id": 1, "name": "Test", "category": "meditation", "frequency": "daily"}
        ]
        completions = [
            {
                "habit_id": 1,
...
class TestHabitAPIEndpoints:
    ...
    def test_check_alignment_invalid_category(self, client):
        ...
        response = client.post("/habits/alignment?category=invalid&moon_phase=new_moon")

        assert response.status_code == 400

    def test_get_recommendations(self, client):
        """Test POST /habits/recommendations endpoint."""
        response = client.post("/habits/recommendations?moon_phase=full_moon")
    ...
    def test_create_habit(self, client):
        ...
        response = client.post(
            "/habits/create", json={"name": "Morning Yoga", "category": "exercise"}
        )
    ...
    def test_create_habit_invalid_category(self, client):
        ...
        response = client.post(
            "/habits/create", json={"name": "Test", "category": "invalid_category"}
        )
    ...
    def test_log_completion(self, client):
        ...
        response = client.post("/habits/log?moon_phase=full_moon", json={"habit_id": 1})
    ...
    def test_get_lunar_habit_recommendations_localized(self):
        with patch("backend.app.engine.habit_tracker.get_translation") as mock_trans:
            ...
            def side_effect(lang, key):
                if key == "habit_phase_new_moon_theme":
                    return ["Nuevos Comienzos"]
                if key == "habit_cat_meditation_name":
                    return ["Meditacion"]
                if key == "habit_why_aligned":
                    return ["Alineado con {theme}"]
                return None
            ...
            med_rec = next(
                (
                    r
                    for r in recs["recommended_categories"]
                    if r["category"] == "meditation"
                ),
                None,
            )
            assert med_rec
