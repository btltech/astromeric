    ...
    return {
        "planets": [
            ...
            },
            {
                ...
                "absolute_degree": 10.0,
                "retrograde": False,
            },  # Early degree, conjunct Sun
            {
                "name": "Mercury",
                "sign": "Aries",
                "degree": 12.0,
                "absolute_degree": 12.0,
                "retrograde": False,
            },
            {
                "name": "Venus",
...
class TestSunriseSunset:
    ...
    def test_calculate_sunrise_sunset_returns_tuple(self, sample_location):
        ...
        date = datetime(
            2024, 6, 21, 12, 0, tzinfo=ZoneInfo(sample_location["timezone"])
        )
...
class TestPlanetaryHours:
    ...
    def test_sunday_starts_with_sun(self, sample_location):
        ...
        date = datetime(
            2024, 6, 23, 12, 0, tzinfo=ZoneInfo(sample_location["timezone"])
        )
        hours = calculate_planetary_hours(
            date,
            sample_location["latitude"],
    ...
    def test_saturday_starts_with_saturn(self, sample_location):
        ...
        date = datetime(
            2024, 6, 22, 12, 0, tzinfo=ZoneInfo(sample_location["timezone"])
        )
        hours = calculate_planetary_hours(
            date,
            sample_location["latitude"],
...
class TestCurrentPlanetaryHour:
    ...
    def test_current_hour_planet_in_chaldean_order(self, sample_location):
        ...
        date = datetime(
            2024, 6, 21, 14, 0, tzinfo=ZoneInfo(sample_location["timezone"])
        )
        result = get_current_planetary_hour(
            date,
            sample_location["latitude"],
...
class TestRetrogrades:
    ...
    def test_detect_retrogrades_with_retrogrades(
        self, sample_transit_chart_with_retrogrades
    ):
    ...
    def test_detect_retrogrades_no_retrogrades(
        self, sample_transit_chart_no_retrogrades
    ):
    ...
    def test_mercury_retrograde_specific_impacts(
        self, sample_transit_chart_with_retrogrades
    ):
        ...
        assert (
            "Communication" in mercury["general_impact"]
            or "delays" in mercury["general_impact"]
        )
...
class TestPlanetaryTimingIntegration:
    ...
    def test_full_timing_calculation(
        self, sample_location, sample_transit_chart_with_retrogrades
    ):
