...
def test_natal_and_compatibility_endpoints():
    ...
    # Compatibility
    resp = client.post('/compatibility', json={'person_a': person_a, 'person_b': person_b})
    assert resp.status_code == 200
    comp = resp.json()
    assert isinstance(comp, dict)
    assert any(k in comp for k in ('topic_scores', 'highlights', 'compatibility', 'astro', 'numerology'))
