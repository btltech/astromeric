...
from app.engine.interpretation.blocks import MeaningBlock, get_meaning_block
...
class TestBlocksLocalization:
    ...
    def test_get_meaning_block_planet_sign_localized(self, mock_get_text):
        ...
        block = get_meaning_block(
            mock_dict, "Sun", "Aries", lang="es", context="planet_sign"
        )
    ...
    def test_get_meaning_block_planet_house_localized(self, mock_get_translation):
        ...
        block = get_meaning_block(
            mock_dict, "Sun", 1, lang="es", context="planet_house"
        )

        assert block.text == "Sol en Casa 1 (ES)"
        mock_get_translation.assert_called_with(
            "es", "planet_house_meanings", "Sun_house_1"
        )
    ...
    def test_get_meaning_block_aspect_localized(self, mock_get_translation):
        ...
        mock_get_translation.assert_called_with(
            "es", "aspect_meanings", "aspect_conjunction"
        )
