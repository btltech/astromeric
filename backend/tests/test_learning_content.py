...
def test_get_moon_sign_lesson_localized():
    with patch("app.engine.learning_content.get_translation") as mock_trans:
        ...
        def side_effect(lang, key):
            if key == "learn_moon_aries_title":
                return ["Luna en Aries"]
            if key == "learn_moon_aries_short":
                return ["Naturaleza de fuego"]
            return None
...
def test_get_lesson_localized():
    with patch("app.engine.learning_content.get_translation") as mock_trans:
        ...
        def side_effect(lang, key):
            if key == "learn_lesson_read_your_chart_1_title":
                return ["Los Tres Grandes"]
            return None
...
def test_search_learning_content_localized():
    with patch("app.engine.learning_content.get_translation") as mock_trans:
        ...
        aries_moon = next(
            (r for r in results if r["type"] == "moon_sign" and r["key"] == "Aries"),
            None,
        )
        assert aries_moon
