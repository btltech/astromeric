...
class TestActivityProfiles:
    ...
    def test_all_activities_have_required_fields(self):
        ...
        for activity, profile in ACTIVITY_PROFILES.items():
            for field in required_fields:
                assert (
                    field in profile
                ), f"Activity '{activity}' missing field '{field}'"
    ...
    def test_activities_have_valid_planets(self):
        ...
        for activity, profile in ACTIVITY_PROFILES.items():
            for planet in profile["favorable_planets"]:
                assert (
                    planet in valid_planets
                ), f"Activity '{activity}' has invalid planet '{planet}'"
            for planet in profile["unfavorable_planets"]:
                assert (
                    planet in valid_planets
                ), f"Activity '{activity}' has invalid unfavorable planet '{planet}'"
    ...
    def test_activities_have_valid_moon_phases(self):
        ...
        for activity, profile in ACTIVITY_PROFILES.items():
            for phase in profile["favorable_moon_phases"]:
                ...
                ), f"Activity '{activity}' has invalid moon phase '{phase}'"
            for phase in profile["unfavorable_moon_phases"]:
                assert (
                    phase in valid_phases
                ), f"Activity '{activity}' has invalid unfavorable moon phase '{phase}'"
    ...
    def test_expected_activities_exist(self):
        ...
        for activity in expected:
            assert (
                activity in ACTIVITY_PROFILES
            ), f"Expected activity '{activity}' not found"
    ...
    def test_weight_factors_sum_to_one(self):
        ...
        for activity, profile in ACTIVITY_PROFILES.items():
            ...
            assert (
                0.99 <= total <= 1.01
            ), f"Activity '{activity}' weights sum to {total}, expected ~1.0"
    ...
    def test_activity_names_are_human_readable(self):
        ...
        for activity, profile in ACTIVITY_PROFILES.items():
            ...
            assert (
                " " in name or name.istitle()
            ), f"Activity '{activity}' name '{name}' should be human-readable"
...
class TestCalculateTimingScore:
    ...
    def test_personal_day_affects_score(self, sample_transit_chart):
        ...
        assert (
            result_good_day["breakdown"]["personal_day"]
            > result_bad_day["breakdown"]["personal_day"]
        )
        assert result_good_day["score"] > result_bad_day["score"]

        assert any("Personal Day" in r for r in result_good_day["recommendations"])
        assert any("Personal Day" in w for w in result_bad_day["warnings"])
...
class TestRetrogradeEffect:
    ...
    def test_mercury_retrograde_affects_contracts(self):
        ...
        assert (
            score_retro["breakdown"]["retrograde"]
            < score_no_retro["breakdown"]["retrograde"]
        )
...
class TestAPIEndpoints:
    ...
    def test_activities_list_endpoint(self):
        ...
        response = client.get("/v2/timing/activities")
    ...
    def test_endpoint_with_profile(self):
        ...
        assert "personal_day" in data["today"]["breakdown"]
