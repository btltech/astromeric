...
def test_astrology_guidance_fire_moon():
    ...
    natal = {}
    transit = {
        "planets": [{"name": "Moon", "sign": "Aries"}],  # Fire sign
        "aspects": [],
    }
...
def test_astrology_guidance_water_moon():
    ...
    natal = {}
    transit = {
        "planets": [{"name": "Moon", "sign": "Pisces"}],  # Water sign
        "aspects": [],
    }
...
def test_astrology_guidance_earth_moon():
    ...
    natal = {}
    transit = {"planets": [{"name": "Moon", "sign": "Taurus"}], "aspects": []}
...
def test_astrology_guidance_air_moon():
    ...
    natal = {}
    transit = {"planets": [{"name": "Moon", "sign": "Gemini"}], "aspects": []}
...
def test_full_guidance_integration():
    ...
    numerology = {
        "cycles": {"personal_day": {"number": 4}},
        "challenges": [{"number": 1}, {"number": 2}],
    }
...
def test_missing_cycles():
    ...
    transit = {
        "planets": [
            {"name": "Moon", "sign": "Taurus", "degree": 10, "absolute_degree": 40}
        ],
        "aspects": [],
    }
...
def test_calculate_power_hour_en():
    ...
    with patch("app.engine.guidance.get_power_hours") as mock_ph:
        ...
        result = _calculate_power_hour(
            natal, transit, latitude=40.7, longitude=-74.0, lang="en"
        )
...
def test_get_current_hour_info_en():
    with patch("app.engine.guidance.get_current_planetary_hour") as mock_ph:
        mock_ph.return_value = {"planet": "Sun", "start": "10:00", "end": "11:00"}
...
def test_get_daily_guidance_integration_localized():
    ...
    with patch("app.engine.guidance._get_numerology_guidance") as mock_num, patch(
        "app.engine.guidance._get_astrology_guidance"
    ) as mock_astro, patch(
        "app.engine.guidance._get_color_guidance"
    ) as mock_color, patch(
        "app.engine.guidance._calculate_power_hour"
    ) as mock_power, patch(
        "app.engine.guidance._get_current_hour_info"
    ) as mock_hour:
        ...
        mock_power.assert_called_with(
            natal, transit, latitude=40.7, longitude=-74.0, timezone="UTC", lang="es"
        )
