...
# VAPID constants
VAPID_PUBLIC_KEY = os.getenv("VAPID_PUBLIC_KEY", "BCPloRz8zIkfeMDNuaqTN5bIreZsr6sYcKIBqbQV3X3TiHBpd3GvGRngnPgF4U4QUWBbQToMHYa9HoVJ8v_gGxo")
VAPID_PRIVATE_KEY = os.getenv("VAPID_PRIVATE_KEY", "YxDywGzrvW8Wra7lzPknQXfZhPd3vv2Cgzoe_I1S-q8")
VAPID_CLAIMS = {
    "sub": "mailto:alerts@astromeric.com"
}
...
def broadcast_transit_alert(
    ...
    if alert_type == "mercury_retrograde":
        users_to_notify = db.query(User).filter(
            User.alert_mercury_retrograde == True
        ).all()
