...
class RateLimiter:
    ...
    def is_allowed(self, request: Request) -> Tuple[bool, Dict]:
        ...
        headers = {
            ...
            "X-RateLimit-Reset": str(int(time.time() + (self.burst_size - self.tokens[client_id]) / self.rate)),
        }
...
strict_limiter = RateLimiter(requests_per_minute=10, burst_size=5)  # For expensive operations
