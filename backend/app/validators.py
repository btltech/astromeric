...
def validate_time_of_birth(
    time_str: Optional[str], field_name: str = "time_of_birth"
) -> Optional[str]:
    """
    ...
    try:
        ...
        raise ValidationError(
            field_name, f"Invalid time: {time_str}", "Please use HH:MM format (24-hour)"
        )

    return normalized


def validate_timezone(tz: Optional[str], field_name: str = "timezone") -> str:
    """
...
def validate_house_system(
    system: Optional[str], field_name: str = "house_system"
) -> str:
    """
...
def validate_name(name: Optional[str], field_name: str = "name") -> str:
    ...
    if not name or not name.strip():
        raise ValidationError(
            field_name, "Name is required", "Please provide a name for the profile"
        )
