...
# Venus and Mars sign changes for 2024-2026 (approximate dates)
# These are major relationship transits — extended through end of 2026
VENUS_INGRESSES_2024_2025 = VENUS_INGRESSES = [
    ...
    # 2026 — Venus direct (no retrograde until late 2026)
    {"date": "2026-01-18", "sign": "Aquarius", "end": "2026-02-11"},
    {"date": "2026-02-11", "sign": "Pisces", "end": "2026-03-07"},
    {"date": "2026-03-07", "sign": "Aries", "end": "2026-04-02"},
    {"date": "2026-04-02", "sign": "Taurus", "end": "2026-04-28"},
    {"date": "2026-04-28", "sign": "Gemini", "end": "2026-05-23"},
    {"date": "2026-05-23", "sign": "Cancer", "end": "2026-06-17"},
    {"date": "2026-06-17", "sign": "Leo", "end": "2026-07-12"},
    {"date": "2026-07-12", "sign": "Virgo", "end": "2026-08-07"},
    {"date": "2026-08-07", "sign": "Libra", "end": "2026-09-01"},
    {"date": "2026-09-01", "sign": "Scorpio", "end": "2026-09-27"},
    {"date": "2026-09-27", "sign": "Sagittarius", "end": "2026-10-23"},
    {"date": "2026-10-23", "sign": "Capricorn", "end": "2026-11-17"},
    {"date": "2026-11-17", "sign": "Aquarius", "end": "2026-12-12"},
    {"date": "2026-12-12", "sign": "Pisces", "end": "2027-01-06"},
]

MARS_INGRESSES_2024_2025 = MARS_INGRESSES = [
    ...
    # 2026
    {"date": "2026-01-24", "sign": "Aquarius", "end": "2026-03-06"},
    {"date": "2026-03-06", "sign": "Pisces", "end": "2026-04-18"},
    {"date": "2026-04-18", "sign": "Aries", "end": "2026-05-31"},
    {"date": "2026-05-31", "sign": "Taurus", "end": "2026-07-13"},
    {"date": "2026-07-13", "sign": "Gemini", "end": "2026-08-27"},
    {"date": "2026-08-27", "sign": "Cancer", "end": "2026-10-11"},
    {"date": "2026-10-11", "sign": "Leo", "end": "2026-11-25"},
    {"date": "2026-11-25", "sign": "Virgo", "end": "2027-01-09"},
]

# Venus retrograde periods (challenging for new relationships)
VENUS_RETROGRADES = [
    {"start": "2025-03-01", "end": "2025-04-12", "sign": "Aries/Pisces"},
    # Next Venus retrograde: Oct 2026 – Nov 2026 in Scorpio/Libra
    {"start": "2026-10-03", "end": "2026-11-14", "sign": "Scorpio/Libra"},
]
...
RELATIONSHIP_ECLIPSES = [
    {
        ...
        "type": "Lunar",
        "sign": "Libra",
        "impact": "Partnership revelations",
    },
    {
        "date": "2024-10-02",
        "type": "Solar",
        "sign": "Libra",
        "impact": "New relationship beginnings",
    },
    {
        "date": "2025-03-14",
        "type": "Lunar",
        "sign": "Virgo",
        "impact": "Service in relationships",
    },
    {
        "date": "2025-03-29",
        "type": "Solar",
        "sign": "Aries",
        "impact": "Individual identity in partnership",
    },
    {
        "date": "2025-09-07",
        "type": "Lunar",
        "sign": "Pisces",
        "impact": "Spiritual connection",
    },
    {
        "date": "2025-09-21",
        "type": "Solar",
        "sign": "Virgo",
        "impact": "Practical relationship matters",
    },
    # 2026 eclipses
    {
        "date": "2026-02-17",
        "type": "Solar",
        "sign": "Aquarius",
        "impact": "Innovation in relationships",
    },
    {
        "date": "2026-03-03",
        "type": "Lunar",
        "sign": "Virgo",
        "impact": "Healing and service in love",
    },
    {
        "date": "2026-08-12",
        "type": "Solar",
        "sign": "Leo",
        "impact": "Creative romance and self-expression",
    },
    {
        "date": "2026-08-28",
        "type": "Lunar",
        "sign": "Pisces",
        "impact": "Deep emotional release in partnerships",
    },
]
...
RELATIONSHIP_HOUSE_PHASES = {
    5: {
        ...
        "description": "Creative self-expression, fun, flirtation",
    },
    7: {
        "theme": "Committed Partnership",
        "description": "Marriage, long-term commitment, balance",
    },
    8: {
        "theme": "Deep Intimacy",
        "description": "Transformation, shared resources, emotional depth",
    },
    4: {
        "theme": "Building Home",
        "description": "Family, domestic life, emotional foundation",
    },
    11: {
        "theme": "Friendship & Community",
        "description": "Shared ideals, social connections",
    },
}
...
def is_venus_retrograde(date: datetime, lang: str = "en") -> Dict[str, Any]:
    ...
    for retrograde in VENUS_RETROGRADES:
        if retrograde["start"] <= date_str <= retrograde["end"]:
            datetime.strptime(retrograde["start"], "%Y-%m-%d")
            end_dt = datetime.strptime(retrograde["end"], "%Y-%m-%d")
            # Strip timezone to compare against naive parsed dates
            date_naive = date.replace(tzinfo=None) if date.tzinfo else date
            days_remaining = (end_dt - date_naive).days

            warning_trans = get_translation(lang, "venus_retro_warning")
            warning = (
                warning_trans[0]
                if warning_trans
                else "Venus retrograde is not ideal for starting new relationships or making major romantic decisions. Focus on reflection and reconnecting."
            )
    ...
    message = (
        msg_trans[0]
        if msg_trans
        else "Venus is direct - favorable for love and relationships"
    )

    return {"is_retrograde": False, "message": message, "emoji": "💕✨"}
...
def get_upcoming_relationship_dates(
    ...
    for ingress in VENUS_INGRESSES_2024_2025:
        if start_str <= ingress["date"] <= end_str:
            ...
            title = (
                title_trans[0].format(sign=ingress["sign"])
                if title_trans
                else f"Venus enters {ingress['sign']}"
            )

            desc_trans = get_translation(lang, "venus_ingress_desc")
            desc_base = (
                desc_trans[0].format(sign=ingress["sign"])
                if desc_trans
                else f"Love and attraction take on {ingress['sign']} qualities."
            )
            ...
            if is_personal:
                ...
                personal_msg = (
                    personal_trans[0]
                    if personal_trans
                    else "This is your sign - expect increased charm and romantic opportunities!"
                )
                description = f"{desc_base} {personal_msg}"
            else:
                general_trans = get_translation(lang, "venus_ingress_general")
                general_msg = (
                    general_trans[0] if general_trans else "Good for relationships."
                )
    ...
    for ingress in MARS_INGRESSES_2024_2025:
        if start_str <= ingress["date"] <= end_str:
            ...
            title = (
                title_trans[0].format(sign=ingress["sign"])
                if title_trans
                else f"Mars enters {ingress['sign']}"
            )
            ...
            description = (
                desc_trans[0].format(sign=ingress["sign"])
                if desc_trans
                else f"Passion and desire express through {ingress['sign']} energy. Channel energy constructively."
            )
    ...
    for retrograde in VENUS_RETROGRADES:
        if start_str <= retrograde["start"] <= end_str:
            ...
            description = (
                desc_trans[0]
                if desc_trans
                else "Not ideal for starting new relationships. Past lovers may reappear. Reflect on love patterns."
            )
        ...
        if start_str <= retrograde["end"] <= end_str:
            ...
            description = (
                desc_trans[0]
                if desc_trans
                else "Venus goes direct! Love matters clarify and relationships can move forward."
            )
    ...
    for eclipse in RELATIONSHIP_ECLIPSES:
        if start_str <= eclipse["date"] <= end_str:
            ...
            title = (
                title_trans[0].format(type=eclipse["type"], sign=eclipse["sign"])
                if title_trans
                else f"{eclipse['type']} Eclipse in {eclipse['sign']}"
            )

            desc_trans = get_translation(lang, "eclipse_desc")
            desc_suffix = (
                desc_trans[0] if desc_trans else "Major relationship shifts possible."
            )
...
def analyze_relationship_timing(
    ...
    if venus_rx.get("is_retrograde"):
        ...
        warning = (
            warn_trans[0]
            if warn_trans
            else "Venus retrograde - not ideal for new romantic beginnings"
        )
    ...
    if venus:
        ...
        SIGN_RELATIONSHIP_THEMES.get(person1_sign, {})
        ...
        if venus_sign == person1_sign:
            ...
            factor_msg = (
                factor_trans[0].format(sign=person1_sign)
                if factor_trans
                else f"Venus in your sign ({person1_sign})"
            )
            factors.append({"factor": factor_msg, "impact": 15, "emoji": "⭐"})

            rec_trans = get_translation(lang, "rel_timing_venus_sign_rec")
            rec_msg = (
                rec_trans[0].format(sign=person1_sign)
                if rec_trans
                else f"Venus in {person1_sign} enhances your natural charm and attractiveness!"
            )
        ...
        if person_element and venus_element:
            ...
            if venus_element in compatible_elements.get(person_element, []):
                ...
                factors.append(
                    {
                        "factor": f"Venus in compatible {venus_element} element",
                        "impact": 10,
                        "emoji": "✨",
                    }
                )
    ...
    if mars:
        ...
        if mars_sign in ["Aries", "Scorpio"]:  # Mars rules these
            score += 5
            factors.append(
                {"factor": f"Mars strong in {mars_sign}", "impact": 5, "emoji": "🔥"}
            )
            recommendations.append(
                "Passion and initiative are heightened - take action!"
            )
...
def get_best_relationship_days(
    start_date: datetime, sun_sign: str, days_ahead: int = 30
) -> List[Dict[str, Any]]:
    """
...
def build_relationship_timeline(
    ...
    period_scores = [
        analyze_relationship_timing(start_date + timedelta(days=i), sun_sign)["score"]
