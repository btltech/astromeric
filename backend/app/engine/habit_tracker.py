...
LUNAR_HABIT_GUIDANCE = {
    "new_moon": {
        ...
        "avoid": ["Major launches", "High-intensity workouts", "Overcommitting"],
    ...
    "first_quarter": {
        ...
        "avoid": ["Procrastination", "Being too cautious"],
    ...
    "waxing_gibbous": {
        ...
        "avoid": ["Making major changes", "Starting over"],
    ...
    "full_moon": {
        ...
        "avoid": ["Starting brand new habits", "Rash decisions"],
        "energy": "Peak, intense",
    ...
    "waning_gibbous": {
        ...
        "avoid": ["Overextending", "Ignoring rest needs"],
    ...
    "last_quarter": {
        ...
        "avoid": ["Starting new projects", "Clinging to old patterns"],
    ...
    "waning_crescent": {
        ...
        "avoid": ["Pushing too hard", "Major new initiatives"],
...
def create_habit(
    ...
    if lang != "en":
        ...
        if cat_name_trans:
            cat_info["name"] = cat_name_trans[0]
...
def calculate_lunar_alignment_score(
    habit_category: str, moon_phase: str
) -> Dict[str, Any]:
    """
...
def get_habit_streak(
    completions: List[Dict[str, Any]], frequency: str = "daily"
) -> Dict[str, Any]:
    """
    ...
    sorted_completions = sorted(
        completions, key=lambda x: x.get("date", ""), reverse=True
    )

    # Get unique dates
    unique_dates = sorted(
        set(c.get("date", "") for c in sorted_completions), reverse=True
    )
...
def get_lunar_habit_recommendations(
    moon_phase: str, existing_habits: List[Dict[str, Any]] = None, lang: str = "en"
) -> Dict[str, Any]:
    """
    ...
    for cat_key in ideal_categories:
        if cat_key in HABIT_CATEGORIES:
            ...
            if lang != "en":
                ...
                if cat_name_trans:
                    cat["name"] = cat_name_trans[0]

                cat_desc_trans = get_translation(lang, f"habit_cat_{cat_key}_desc")
                if cat_desc_trans:
                    cat["description"] = cat_desc_trans[0]

            why_trans = get_translation(lang, "habit_why_aligned")
            why = (
                why_trans[0].format(theme=phase_info.get("theme", "this phase"))
                if why_trans
                else f"Aligned with {phase_info.get('theme', 'this phase')} energy"
            )
    ...
    if existing_habits:
        for habit in existing_habits:
            ...
            if cat in ideal_categories:
                ...
                msg = (
                    msg_trans[0] if msg_trans else "Great time to focus on this habit!"
                )

                aligned_existing.append(
                    {"habit_name": habit.get("name"), "category": cat, "message": msg}
                )
...
def calculate_habit_analytics(
    habit: Dict[str, Any], completions: List[Dict[str, Any]], period_days: int = 30
) -> Dict[str, Any]:
    """
    ...
    period_completions = [
        c for c in completions if c.get("completed_at", c.get("date", "")) >= cutoff
    ]
    ...
    best_day = (
        max(by_weekday.keys(), key=lambda k: by_weekday[k]) if by_weekday else None
    )
    ...
    if best_phase:
        phase_name = LUNAR_HABIT_GUIDANCE.get(best_phase, {}).get(
...
def get_today_habit_forecast(
    habits: List[Dict[str, Any]], moon_phase: str, completions_today: List[int] = None
) -> Dict[str, Any]:
    """
...
def get_lunar_cycle_report(
    ...
    avg_rate = sum(h["completion_rate"] for h in habit_reports) / max(
        1, len(habit_reports)
    )
