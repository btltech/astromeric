...
RETROGRADE_IMPACTS = {
    ...
    "Venus": {
        ...
        "activities_embrace": [
            "Reconnect with exes (if healthy)",
            "Reassess values",
            "Self-love practices",
        ],
    },
...
def calculate_sunrise_sunset(
    date: datetime, latitude: float, longitude: float, timezone: str = "UTC"
    ...
    for planet in planets:
        if planet.get("retrograde", False):
            ...
            general_impact = (
                general_trans[0]
                if general_trans
                else impact_data.get("general", f"{name} energy turned inward")
            )
            ...
            activities_avoid = (
                avoid_trans if avoid_trans else impact_data.get("activities_avoid", [])
            )
            activities_embrace = (
                embrace_trans
                if embrace_trans
                else impact_data.get("activities_embrace", [])
            )
...
def calculate_void_of_course_moon(transit_chart: Dict, lang: str = "en") -> Dict:
    ...
    return {
        ...
        "next_sign": next_sign,
        "hours_until_sign_change": (
            round(hours_until_sign_change, 1) if is_void else None
        ),
        "advice": advice,
    }
