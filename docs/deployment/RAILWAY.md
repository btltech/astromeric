...
The shared Railway backend now supports Android FCM delivery and exposes an authenticated smoke route at `/v2/alerts/test-fcm`.

To wire Firebase Admin into Railway:

```bash
./scripts/configure_railway_firebase_push.sh
./scripts/configure_railway_firebase_push.sh astromeric-260422-f3f5 /absolute/path/to/service-account.json
```

The first command sets the non-secret project id. The second also uploads the service-account JSON into `FIREBASE_SERVICE_ACCOUNT_JSON` for the linked Railway service.
