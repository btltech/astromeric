...
export function downloadReadingPdf(data: PredictionData): void {
  ...
  const addFooter = () => {
    ...
    doc.text(new Date().toLocaleDateString(), pageWidth - margin, pageHeight - 10, {
      align: 'right',
    });
  };
  ...
  const profileText = [
    sunSign,
    moonSign ? `Moon in ${moonSign}` : null,
  ...
  doc.setTextColor(...COLORS.muted);
  const disclaimer =
    'This reading is for entertainment and self-reflection purposes. The insights provided are based on astrological and numerological traditions and should not replace professional advice.';
  ...
  // Download
  const fileName = `astronumeric-premium-${data.scope}-${
