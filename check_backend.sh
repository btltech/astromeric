...
API_TEST=$(curl -s -X POST https://astromeric-backend-production.up.railway.app/daily-reading \
...
CORS=$(curl -s -I -X OPTIONS \
  ...
  https://astromeric-backend-production.up.railway.app/daily-reading 2>&1 | grep -i "access-control-allow-origin")
